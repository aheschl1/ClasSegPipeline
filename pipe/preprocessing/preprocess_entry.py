...
class Preprocessor:
    def __init__(
            self, dataset_id: str, folds: int, processes: int, normalize: bool,
            dataset_desc: str = None, **kwargs
    ):
        """
    ...
    def process(self) -> None:
        ...
        assert (
                len(self.datapoints) >= 1
        ), f"There are no datapoints in {RAW_ROOT}/{self.dataset_name}"
    ...
    def verify_dataset_integrity(self) -> None:
        ...
        for point in tqdm(self.datapoints, desc="Verifying dataset integrity"):
            ...
            assert names_before < len(
                names
            ), f"The name {point.case_name} is in the dataset at least 2 times :("
    ...
    def process_fold(self, fold: int) -> None:
        ...
        if self.mode == SEGMENTATION:
            print("WARNING: BINARY SEGMENTATION")
        ...
        for _set in ["train", "val"]:
            for images, labels, points in tqdm(
                ...
                if self.normalize and self.mode in [SELF_SUPERVISED, SEGMENTATION]:
...
def get_preprocessor_from_name(name: str) -> Type[Preprocessor]:
    from pipe.preprocessing.mnist_preprocessor import MnistPreprocessor
    from pipe.preprocessing.wheathead_preprocessor import WheatheadPreprocessor
    return {
        BASE: Preprocessor,
        MNIST: MnistPreprocessor,
        WHEAT: WheatheadPreprocessor
    }[name]
...
@click.option("-data_path", "-dp", help="The path to segmentation image pair paths", type=str)
@click.option("-dataset_desc", "-dd", help="The dataset name", type=str, default=None)
@click.option("-preprocessor", help="Identifier of the preprocessor you want to use", default="base")
def main(folds: int, processes: int, normalize: bool,
         dataset_id: str, data_path: str, preprocessor: str, dataset_desc: str):
    assert preprocessor in [BASE, MNIST, WHEAT], f"Only {BASE}, {WHEAT}, and {MNIST} are supported preprocessors."
    preprocessor = get_preprocessor_from_name(preprocessor)
    preprocessor = preprocessor(
        dataset_id=dataset_id, normalize=normalize, folds=folds, processes=processes,
        data_path=data_path, dataset_desc=dataset_desc
    )
